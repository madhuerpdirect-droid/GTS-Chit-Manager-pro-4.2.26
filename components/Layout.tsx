...
const Layout: React.FC<Props> = ({ children, activePage, setActivePage, user, onLogout, onSync, isDirty }) => {
  ...
  const navContent = (
    <>
      <div className="p-6 flex items-center justify-between">
        <h1 className="text-xl font-bold ms-primary flex items-center gap-2">
      ...
      <div className="p-4 border-t ms-border">
        <div className="flex items-center gap-3 p-2">
          <div className="w-8 h-8 rounded-full ms-bg-primary text-white flex items-center justify-center text-xs font-bold">
            {user.name.charAt(0)}
          </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white min-w-0">
        <header className="h-14 border-b ms-border flex items-center justify-between px-4 md:px-6 bg-white shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={onSync}
              className={`flex items-center gap-2 px-3 py-1.5 text-xs font-bold border rounded transition-all ${
                isDirty 
                ? 'bg-amber-500 border-amber-600 text-white hover:bg-amber-600 shadow-md scale-105' 
                : 'border-gray-200 text-gray-500 hover:bg-gray-50'
              }`}
              title={isDirty ? "Unsaved changes! Click to Sync" : "All data synced"}
            >
              <svg className={`w-4 h-4 ${isDirty ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              {isDirty ? 'Sync Pending' : 'Synced'}
            </button>
