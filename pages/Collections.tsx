...
const Collections: React.FC = () => {
  ...
  return (
    <div className="space-y-6 px-1 md:px-0">
      {/* Filters */}
      <div className="ms-bg-card p-4 rounded border ms-border ms-shadow flex flex-col md:flex-row items-stretch md:items-end gap-4">
        <div className="flex flex-col flex-1">
          ...
          <select 
            ...
            className="border ms-border p-2 rounded text-sm outline-none w-full"
          >
                ...
                <tr key={gm.groupMembershipId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button 
                        onClick={() => member && currentChit && sendPaymentLink(currentChit.upiId, member.mobile, member.name, currentChit.name, selectedMonth, balance)}
                        disabled={balance <= 0}
      ...
      <div className="md:hidden space-y-4 pb-20">
        {memberships.map(gm => {
          ...
          const { due, paid, balance, status } = getInstallmentStatus(selectedChit, gm.memberId, selectedMonth);
          return (
            <div key={gm.groupMembershipId} className="ms-bg-card p-4 rounded-lg border ms-border ms-shadow space-y-3">
              <div className="flex justify-between items-start border-b pb-2">
                <div>
                  <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">#{gm.tokenNo} • {member?.mobile}</div>
                  <div className="font-bold text-gray-900 leading-tight">{member?.name}</div>
                </div>
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-red-50 text-red-600'}`}>
                  {status}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-2 py-1 text-center border-b pb-3 border-gray-50">
                <div><div className="text-[10px] text-gray-400 uppercase font-bold mb-1">Due</div><div className="text-sm">₹{due}</div></div>
                <div><div className="text-[10px] text-gray-400 uppercase font-bold mb-1">Paid</div><div className="text-sm text-green-600">₹{paid}</div></div>
                <div><div className="text-[10px] text-gray-400 uppercase font-bold mb-1">Bal</div><div className="text-sm text-red-600 font-bold">₹{balance}</div></div>
              </div>
              <div className="flex gap-3 pt-1">
                <button 
                  onClick={() => setShowCollectDialog({ memberId: gm.memberId, name: member?.name, due: balance })}
                  disabled={balance <= 0}
                  className="flex-1 py-2.5 rounded-md text-xs font-bold ms-bg-primary text-white disabled:opacity-20 active:scale-95 transition-transform"
                >
                  Collect Payment
                </button>
                <button 
                  onClick={() => member && currentChit && sendPaymentLink(currentChit.upiId, member.mobile, member.name, currentChit.name, selectedMonth, balance)}
                  disabled={balance <= 0}
                  className="px-4 py-2.5 border ms-border rounded-md text-green-600 flex items-center justify-center disabled:opacity-20 active:scale-95 transition-transform"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12.031 6.172c-3.181 0-5.767 2.586-5.767 5.767 0 1.267.405 2.436 1.096 3.39l-.79 2.885 2.955-.775a5.727 5.727 0 002.506.582c3.181 0 5.767-2.586 5.767-5.767 0-3.181-2.586-5.767-5.767-5.767zM12 2C6.477 2 2 6.477 2 12c0 1.891.524 3.662 1.435 5.176L2.05 22l4.957-1.301C8.42 21.515 10.138 22 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2z"/></svg>
                </button>
              </div>
        ...
        {memberships.length === 0 && <div className="text-center py-20 text-gray-400 text-sm italic">No members found in this group.</div>}
      </div>
      ...
      {showCollectDialog && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="ms-bg-card p-6 rounded-xl w-full max-w-md ms-shadow animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-bold mb-5 border-b pb-3">Collect Payment: {showCollectDialog.name}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Payment Amount (₹)</label>
                <input type="number" id="payAmount" defaultValue={showCollectDialog.due} className="w-full border ms-border p-2.5 rounded text-sm font-semibold" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Mode</label>
                <select id="payMode" className="w-full border ms-border p-2.5 rounded text-sm">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Reference (Optional)</label>
                <input type="text" id="payRef" placeholder="Transaction ID or Note" className="w-full border ms-border p-2.5 rounded text-sm" />
              </div>
              <div className="flex gap-3 pt-4">
                <button onClick={() => setShowCollectDialog(null)} className="flex-1 py-2.5 border rounded-md font-semibold text-gray-600 hover:bg-gray-50">Cancel</button>
                ...
                })} className="flex-1 py-2.5 ms-bg-primary text-white rounded-md font-bold shadow-md active:scale-95 transition-transform">Confirm Payment</button>
              </div>
            </div>
          </div>
        </div>
